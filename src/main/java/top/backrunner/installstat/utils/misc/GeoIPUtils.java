package top.backrunner.installstat.utils.misc;
...
public class GeoIPUtils {
    ...
    public void init() {
        try {
            ...
            System.out.println("Geo IP database initialized.");
        } catch (Exception e) {
    ...
    public String getCountry(byte[] ip) {
        ...
        if (ipFile == null)
            return BAD_IP_FILE;
        // 保存ip，转换ip字节数组为字符串形式
        String ipStr = GeoIPStringUtils.getIpStringFromBytes(ip);
    ...
    public String getCountry(String ip) {
        return getCountry(GeoIPStringUtils.getIpByteArrayFromString(ip));
    }
    ...
    public String getArea(final String ip) {
        return getArea(GeoIPStringUtils.getIpByteArrayFromString(ip));
    }
    ...
    private String readString(long offset) {
        try {
            ...
            if (i != 0)
                return GeoIPStringUtils.getString(buf, 0, i, "GBK");
        } catch (IOException e) {
            log.error(e.getMessage(), e);
        }
        return "";
